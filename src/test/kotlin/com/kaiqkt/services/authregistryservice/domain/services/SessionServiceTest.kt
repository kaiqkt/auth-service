...
class SessionServiceTest {
    ...
    fun `given a session, should persist successfully`() {
        ...
        val refreshToken = randomSixCharNumber()
    ...
    fun `given a userId, when exist sessions for his, should return the sessions`() {
        ...
        every { sessionRepository.findAllByUserId(any()) } returns sessions

        sessionService.findAllByUserId("01GFPPTXKZ8ZJRG8MF701M0W99")

        verify { sessionRepository.findAllByUserId("01GFPPTXKZ8ZJRG8MF701M0W99") }
    }

    @Test
    fun `given a userId, when not exist sessions for his, should throw SessionNotFoundException`() {

        every { sessionRepository.findAllByUserId(any()) } returns null
    ...
    fun `given a userId, when fail to get the sessions, should throw PersistenceException `() {
        every { sessionRepository.findAllByUserId(any()) } throws PersistenceException("Unable to persist session")
